...
from .scoring import SCORING_ENGINE_VERSION, assess_fit
...
def build_opportunity_brief(request: OpportunityBriefRequest) -> OpportunityBriefResponse:
    ...
    snapshots = sorted({snapshot for item in ranked for snapshot in item.opportunity.sourceSnapshotIds})
    provenance = Provenance(
        ...
        opportunityVersions={(item.opportunity.canonicalId or item.opportunity.id): item.opportunity.version for item in ranked},
        engineVersion=SCORING_ENGINE_VERSION,
    )
    summary = (
        f"Evaluated {len(request.opportunities)} versioned opportunity dossier(s). "
