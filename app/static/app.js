...
function deadlineText(date){ if(!date)return 'Open'; const days=Math.ceil((new Date(date+'T23:59:59Z')-new Date())/86400000); if(days<0)return 'Closed'; if(days===0)return 'Today'; if(days===1)return '1 day'; return `${days} days`; }
...
function renderFeed(filter='all'){
  ...
  list.innerHTML=items.map((o,i)=>{
    const a=state.assessments.get(o.id); return `<article class="opportunity ${state.selected?.id===o.id?'selected':''}" data-id="${o.id}">
      <div class="rank">${String(i+1).padStart(2,'0')}</div>
      <div class="opp-title"><span class="opp-type">${o.type} · ${sourceBadge(o)}</span><h3>${o.title}</h3><p>${o.issuer} — ${o.summary}</p></div>
      <div class="opp-meta"><p>${o.requirements.length} requirements<br>${o.applicationEffort} preparation effort</p></div>
...
function renderAssessment(){
  ...
  $('#score-value').textContent=a.score; $('#score-opportunity').textContent=`${state.selected.issuer} / ${state.selected.title}`;
...
function renderGaps(){
  const a=state.assessments.get(state.selected.id);
  $('#gaps-list').innerHTML=a.gaps.map(g=>`<div class="gap-row"><div class="gap-status ${g.status}">${g.status}</div><strong>${g.requirement}</strong><p>${g.recommendedAction||g.reason}</p></div>`).join('')||'<div class="empty">No explicit requirements supplied.</div>';
}
...
function renderProfile(){
  ...
  $('#evidence-count').textContent=`${state.profile.evidence.length} records`;
  $('#evidence-list').innerHTML=state.profile.evidence.map(e=>`<div class="evidence-row"><span>${e.label}<br><small>${e.kind}</small></span><span class="tag ${e.status}">${e.status}</span></div>`).join('')||'<div class="empty">No verified evidence yet. Add repository, deployment, test, transaction, demo or documentation records through the API.</div>';
}
async function saveProfile(){
  ...
  state.profile=await api('/api/profile',{method:'PUT',body:JSON.stringify(state.profile)}); state.assessments.clear(); renderProfile(); renderFeed(); updateMetrics(); toast('Norn Profile saved');
}
